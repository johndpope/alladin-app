...
class Cminds_Marketplace_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return (string) Mage::getConfig()->getNode()->modules->Cminds_Marketplace->version;
    }
...
//            throw new Mage_Exception('Cminds Core Module is disabled or removed');
    ...
    {
        return Mage::getBaseDir('media') . DS . 'supplier_logos' . DS;
    }

    public function getLogosUrl()
    {
        return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'supplier_logos' . DS;
    }
    ...
    public function getSupplierLogo($supplier_id = false)
    {
        ...
        $fileWithPath  = $this->getLogosPath() . $file;
        if (!file_exists($fileWithPath)) {
            return false;
        }

        return $this->getLogosUrl() . $file;
    }
    ...
    public function removeSciptTags($htmlString)
    {
        return str_replace(array('<script>','</script>','&lt;script&gt;','&lt;/script&gt;'), '', $htmlString);
