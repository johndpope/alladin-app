...
class Cminds_Supplierfrontendproductuploader_Helper_Email extends Mage_Core_Helper_Abstract
{
    ...
    {
        return $this->getConfig('show_prices');
    }
    ...
    /**
     * Send Email for a new supplier.
     *
     * @param $supplier
     */
    public function welcomeNewSupplier($supplier)
    {
        $emailTo = Mage::getModel('core/email_template');
        $emailTo->loadDefault('supplier_new_welcome');
        $emailTo->setTemplateSubject(
            'Welcome Supplier ' . $supplier->getFirstname() . ' ' . $supplier->getLastname()
        );
