...
class Cminds_Supplierfrontendproductuploader_Controller_Action extends Mage_Core_Controller_Front_Action {
    ...
    protected function _renderBlocks($isForm = false, $useCkeditor = false,$useDropdownZone = false) {
        ...
        if ($supplierHelper->includeBootstrapJs()) {
        ...
        if($useCkeditor) {
            $this->getLayout()->getBlock('head')->addItem('skin_js', 'js/supplierfrontendproductuploader/ckeditor/ckeditor.js');
        }

        if($useDropdownZone) {
            $this->getLayout()->getBlock('head')->addItem('skin_js', 'js/supplierfrontendproductuploader/jquery.MultiFile.pack.js');
        }
