...
class Cminds_Supplierfrontendproductuploader_Block_Product_Associated extends Cminds_Supplierfrontendproductuploader_Block_Product_Create
{
    ...
    /**
     * Determine whether a product with such attributes is present
     *
     * @param $simpleProduct
     * @return bool
     */
    ...
    {
        $configurableValues = $this->getConfigurableModel()
            ->getConfigurableProductValues();
        $product = Mage::getModel('catalog/product')
            ->load($simpleProduct->getId());
        $simpleProductValue = $this->getConfigurableModel()
            ->getSimpleProductValues($product);

        $attributeCount = count($simpleProductValue);

        foreach ($configurableValues as $itemValues) {
            $inSimpleAtributeCount = 0;

            foreach ($simpleProductValue as $simpleItem) {
                foreach ($itemValues as $item) {
                    $diffArray = array_diff_assoc($simpleItem, $item);

                    if (empty($diffArray)) {
                        $inSimpleAtributeCount++;
                    }
                }

                if ($inSimpleAtributeCount == $attributeCount) {
                    return true;
                }
            }
        }
