...
abstract class Fooman_PdfCustomiser_Helper_Pdf extends Mage_Core_Helper_Abstract
{
    ...
    protected $_suppressPageNum;

    /**
     * @return  boolean
     * @access public
     */
    public function getSuppressNum()
    {
        return $this->_suppressPageNum;
    }
     ...
     * @access public
     */
    public function setSuppressNum($bool)
    {
        $this->_suppressPageNum = $bool;
    }
    ...
    public function OutputCustomerOrderComment()
    {
        ...
        //FME_QuickCheckout
        if ((string)Mage::getConfig()->getModuleConfig('FME_QuickCheckout')->active == 'true') {
            if($this->getOrder()->getQuickcheckoutComment()) {
                $orderComments[] = array(
                    'title' => Mage::helper('quickcheckout')->getQuickCheckoutConfig('general/label_comments'),
                    'comment' => Mage::helper('pdfcustomiser')->escapeHtml($this->getOrder()->getQuickcheckoutComment())
                );
            }
        }
                ...
                if (is_array($value)) {
                    $orderComments[] = array(
                        ...
                        'comment' => str_replace(
                            "\n",
                            '<br/>',
                            '<b>' . $label . '</b> ' . Mage::helper('pdfcustomiser')->escapeHtml(implode("\n", $value))
                        )
                    );
                } else {
                    $orderComments[] = array(
                        'title'   => '',
                        'comment' => '<b>' . $label . '</b> ' . Mage::helper('pdfcustomiser')->escapeHtml($value)
                    );
                }
            }
        }
        ...
        if ($order->getGomageCheckoutCustomerComment()) {
            $orderComments[] = array(
                'title' => Mage::helper('pdfcustomiser')->__('Order Comment'),
                'comment' => nl2br(Mage::helper('pdfcustomiser')->escapeHtml($order->getGomageCheckoutCustomerComment()))
            );
        }
