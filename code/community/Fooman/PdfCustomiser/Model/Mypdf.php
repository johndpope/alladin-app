...
class Fooman_PdfCustomiser_Model_Mypdf extends TCPDF
{
        ...
        foreach ($printItem->getAllItems() as $item) {
            ...
            if ($parentItemId) {
                ...
                if ($parentType == 'configurable') {
                    $pdfTemp['qty_detailed'] = $this->_pdfItems[$parentItemId]['qty_detailed'];
                }
            }
            ...
            if ($displayFptInclusivePrices) {
                ...
                $pdfTemp['rowTotal2'] += $item->getWeeeTaxAppliedRowAmount();
                $pdfTemp['subtotal'] += $item->getWeeeTaxAppliedRowAmount();
            } else {
                $pdfTemp['weee'] = false;
            }
            ...
            if ($displayFptInclusivePrices) {
                ...
                $pdfTemp['baseRowTotal2'] += $item->getBaseWeeeTaxAppliedRowAmount();
                $pdfTemp['baseSubtotal'] += $item->getBaseWeeeTaxAppliedRowAmount();
            }
            ...
            if (Mage::helper('core')->isModuleEnabled('Enterprise_GiftWrapping')) {
                //giftwrapping data
                $pdfTemp['giftWrapping'] = array(
                    'gwId'                    => $orderItem->getGwId(),
                    'gwBasePrice'             => $orderItem->getGwBasePrice(),
                    'gwPrice'                 => $orderItem->getGwPrice(),
                    'gwBaseTaxAmount'         => $orderItem->getGwBaseTaxAmount(),
                    'gwTaxAmount'             => $orderItem->getGwTaxAmount(),
                    'gwBasePriceInvoiced'     => $orderItem->getGwBasePriceInvoiced(),
                    'gwPriceInvoiced'         => $orderItem->getGwPriceInvoiced(),
                    'gwBaseTaxAmountInvoiced' => $orderItem->getGwBaseTaxAmountInvoiced(),
                    'gwTaxAmountInvoiced'     => $orderItem->getGwTaxAmountInvoiced()
                );
            }
            ...
            if ($type == 'bundle') {
                ...
                foreach ($bundleChildren as $childItem) {
                    ...
                    $subBundleItem['taxPercent'] = sprintf("%01.2f", $taxPercent) . '%';
                    if ($helper->printProductImages()) {
    ...
    public function Footer()
    {
        ...
        if (Mage::getStoreConfig('sales_pdf/all/allpagenumbers', $helper->getStoreId()) && !$helper->getSuppressNum()) {
            ...
            */
        } elseif($helper->getSuppressNum()) {
            $helper->setSuppressNum(false);
        }
    }
    ...
    public function getItemNameAndSku($item, $helper)
    {
        ...
        if ($attributeCodes) {
            ...
            $product = Mage::getModel('catalog/product')->setStoreId($helper->getStoreId())->load($productId);
            ...
            if ($product) {
                ...
                foreach ($attributeCodes as $attributeCode) {
                    ...
                    if ($attributeCode == 'category_ids') {
                        ...
                        $attribute->setStoreId($helper->getStoreId());
                        if ($attribute) {
    ...
    public function PrepareCustomerAddress(Fooman_PdfCustomiser_Helper_Pdf $helper, $order, $which)
    {
        ...
        if ($which == 'billing') {
            $order->getBillingAddress()->getCountryModel()->unsetData('name');
            $order->getBillingAddress()->getRegionModel()->unsetData('name');
            ...
            if ($order->getCustomerTaxvat()) {
                ...
                if ($customer->getTaxvat()) {
                    $billingAddress .= "<br/>" . Mage::helper('sales')->__('TAX/VAT Number') . ": "
                        . $customer->getTaxvat();
                } elseif ($order->getBillingAddress()->getVatId() !== null) {
                    $billingAddress .= "<br/>" . Mage::helper('sales')->__('TAX/VAT Number') . ": "
            ...
            if (!$order->getIsVirtual()) {
                $order->getShippingAddress()->getCountryModel()->unsetData('name');
                $order->getShippingAddress()->getRegionModel()->unsetData('name');
            ...
            if ($helper->displayWeight()) {
                ...
                foreach ($printItem->getAllItems() as $item) {
                    ...
                    if ($printItem instanceof Mage_Sales_Model_Order) {
                        ...
                        if ($orderItem->getRowWeight() != '0') {
                            $totalWeight += $orderItem->getWeight() * $item->getQty();
                        }
                        else {
                            $totalWeight += $orderItem->getRowWeight();
                        }
                    }
                }
            }
    ...
    private function cmp($a, $b)
    {
        ...
        if (in_array($this->_sortBy, $productDetailsArray)) {
            return strnatcmp($a['productDetails'][$this->_sortBy],$b['productDetails'][$this->_sortBy]);
        }
