    ...
    public function PrepareTotals(Fooman_PdfCustomiser_Helper_Pdf $helper, $salesObject)
    {
        ...
        foreach ($pdfTotals as $pdfTotal) {
            ...
            switch ($pdfTotal->getSourceField()){
                ...
                case 'shipping_amount':
                    ...
                    if ($pdfTotal->canDisplay() && strtoupper($pdfTotal->getSortOrder())!='NO') {
                        if ($salesObject->getShippingInclTax() && $salesObject->getShippingInclTax() != 0) {
                            if ($helper instanceof Fooman_PdfCustomiser_Helper_Pdf_Creditmemo) {
                                $shippingAmount = $salesObject->getShippingAmount() - $salesObject->getShippingTaxAmount();
                                $baseShippingAmount
                                    = $salesObject->getBaseShippingAmount() - $salesObject->getBaseShippingTaxAmount();
                            } else {
                ...
                case 'adjustment_negative':
                    ...
                    break;
                case 'afterpayfee':
                    $amount = $pdfTotal->getOrder()->getAfterpayfeeAmount();
                    if ($amount != 0) {
                        $totals[$sortOrder][] = array(
                            'code'       => $pdfTotal->getSourceField(),
                ...
                case 'fooman_surcharge_amount':
                    ...
                    if ($pdfTotal->canDisplay() && strtoupper($pdfTotal->getSortOrder())!='NO') {
                        if (Mage::getStoreConfig('tax/sales_display/shipping', $helper->getStoreId())
                            ...
                            $totals[$sortOrder][] = array(
                                    'code' => $pdfTotal->getSourceField(),
                                    'label'=>$order->getFoomanSurchargeDescription() . ' ' . Mage::helper('tax')->__('Incl. Tax') .':',
                                    'amount'=>$salesObject->getFoomanSurchargeAmount()
                                        + $salesObject->getFoomanSurchargeTaxAmount(),
                                    'baseAmount'=>$salesObject->getBaseFoomanSurchargeAmount()
                                        + $salesObject->getBaseFoomanSurchargeTaxAmount()
                            ...
                            $totals[$sortOrder][] = array(
                                    'code' => $pdfTotal->getSourceField(),
                                    'label'=>$order->getFoomanSurchargeDescription(). ' ' . Mage::helper('tax')->__('Excl. Tax') .':',
                                    'amount'=>$salesObject->getFoomanSurchargeAmount(),
                                    'baseAmount'=>$salesObject->getBaseFoomanSurchargeAmount()
                            );
                        } else {
                ...
                case 'money_for_points':
                    ...
                    break;
                case 'rewardpoints_earn':
                    //Magestore Reward Points
                    if ($salesObject->getRewardpointsEarn() != 0) {
                        foreach ($pdfTotal->getTotalsForDisplay() as $rewardTotal) {
                            $totals[$sortOrder][] = array(
                                'code'       => $pdfTotal->getSourceField(),
                                'label'      => $rewardTotal['label'],
                                'amount'     => $rewardTotal['amount'],
                                'baseAmount' => $rewardTotal['amount'],
                                'notACurrency' => true
                            );
                        }
                    }
                    ...
                    if ($pdfTotal->canDisplay() && strtoupper($pdfTotal->getSortOrder()) != 'NO') {
                        ...
                        foreach ($tmpPdfTotalAmounts as $tmpPdfTotalAmount) {
                            $totals[$sortOrder][] = array(
                                'code'       => $pdfTotal->getSourceField(),
                                'label'      => $tmpPdfTotalAmount['label'],
                                'amount'     => preg_replace("/([^0-9\\.])/i", "", $tmpPdfTotalAmount['amount']),
                                'baseAmount' => Mage::app()->getStore()->roundPrice(
                                    $pdfTotal->getAmount() * $order->getBaseToOrderRate()
                                )
                            );
                ...
                case 'PaymentFee':
                case 'buckaroofee':
                    if ($salesObject->getBuckarooFee() != 0 && strtoupper($pdfTotal->getSortOrder()) != 'NO') {
                        $paymentmethodCode = $order->getPayment()->getMethod();
                        $totals[$sortOrder][] = array(
                            'code'       => $pdfTotal->getSourceField(),
                            'label'      => Mage::helper('buckaroo3extended')->getfeeLabel($paymentmethodCode) . ':',
                            'amount'     => $salesObject->getBuckarooFee() + $salesObject->getBuckarooFeeTax(),
                            'baseAmount' => $salesObject->getBaseBuckarooFee() + $salesObject->getBaseBuckarooFeeTax()
                        );
                    }
                break;
        ...
        foreach ($totals as $sortOrder) {
            foreach ($sortOrder as $total) {
                ...
                if (isset($total['notACurrency'])) {
                    $formattedTotal['amount_default'] = $total['amount'];
                    $formattedTotal['amount'] = $total['amount'];
                    $formattedTotal['base_amount'] = $total['baseAmount'];
                } else {
