<?php
namespace Zozothemes\Megamenu\Block;
...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        \Zozothemes\Megamenu\Helper\Data $helper,
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null)
    {
        ...
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            foreach($children as $child) {
                ...
                $zozo_menu_hide_item = $cat_model->getData('zozo_menu_hide_item');
                
                if (!$zozo_menu_hide_item) {
                    $sub_children = $this->getActiveChildCategories($child);
                    
                    $zozo_menu_cat_label = $cat_model->getData('zozo_menu_cat_label');
                    $zozo_menu_icon_img = $cat_model->getData('zozo_menu_icon_img');
                    $zozo_menu_font_icon = $cat_model->getData('zozo_menu_font_icon');
                    $zozo_custom_url = $cat_model->getData('zozo_custom_url');
                    ...
                    if($level == 1 && $zozo_menu_icon_img) {
                        $html .= '<div class="menu-thumb-img"><a class="menu-thumb-link" href="'.$url.'"><img src="' . $this->_helper->getBaseUrl().'catalog/category/' . $zozo_menu_icon_img . '" alt="'.$child->getName().'"/></a></div>';
                    }
                    $html .= '<a href="'.$url.'">';
                    if ($level > 1 && $zozo_menu_icon_img)
                        $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $zozo_menu_icon_img . '" alt="'.$child->getName().'"/>';
                    elseif($zozo_menu_font_icon)
                        $html .= '<em class="menu-thumb-icon '.zozo_menu_font_icon.'"></em>';
                    $html .= '<span>'.$child->getName();
                    if($zozo_menu_cat_label)
                        $html .= '<span class="cat-label cat-label-'.$zozo_menu_cat_label.'">'.$this->_megamenuConfig['cat_labels'][$zozo_menu_cat_label].'</span>';
                    $html .= '</span></a>';
                    if(count($sub_children) > 0) {
    ...
    public function getMegamenuHtml()
    {
        ...
        $this->_megamenuConfig = $this->_helper->getConfig('zozo_megamenu');
        ...
        foreach($categories as $category) {
            ...
            if(!$zozo_menu_hide_item) {
                ...
                $zozo_menu_icon_img = $cat_model->getData('zozo_menu_icon_img');
                $zozo_menu_font_icon = $cat_model->getData('zozo_menu_font_icon');
                $zozo_menu_cat_columns = $cat_model->getData('zozo_menu_cat_columns');
                $zozo_menu_float_type = $cat_model->getData('zozo_menu_float_type');
                $zozo_custom_url = $cat_model->getData('zozo_custom_url');
                
                if(!$zozo_menu_cat_columns){
                    $zozo_menu_cat_columns = 4;
                }
                
                $menu_type = $cat_model->getData('zozo_menu_type');
                if(!$menu_type)
                ...
                $zozo_menu_static_width = $cat_model->getData('zozo_menu_static_width');
                if($menu_type=="staticwidth" && $zozo_menu_static_width)
                    $custom_style = ' style="width: '.$zozo_menu_static_width.';"';
                    
                $item_class = 'level0 ';
                $item_class .= $menu_type.' ';
                
                $menu_top_content = $cat_model->getData('zozo_menu_block_top_content');
                $menu_left_content = $cat_model->getData('zozo_menu_block_left_content');
                $menu_left_width = $cat_model->getData('zozo_menu_block_left_width');
                if(!$menu_left_content || !$menu_left_width)
                    $menu_left_width = 0;
                $menu_right_content = $cat_model->getData('zozo_menu_block_right_content');
                $menu_right_width = $cat_model->getData('zozo_menu_block_right_width');
                if(!$menu_right_content || !$menu_right_width)
                    $menu_right_width = 0;
                $menu_bottom_content = $cat_model->getData('zozo_menu_block_bottom_content');
                if($zozo_menu_float_type)
                    $zozo_menu_float_type = 'fl-'.$zozo_menu_float_type.' ';
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content)))
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$item_class.$zozo_menu_float_type.'">';
                if(count($children) > 0) {
                    $html .= '<div class="open-children-toggle"></div>';
                }
                if($zozo_custom_url){
                ...
                if ($zozo_menu_icon_img)
                    $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $zozo_menu_icon_img . '" alt="'.$category->getName().'"/>';
                elseif($zozo_menu_font_icon)
                    $html .= '<em class="menu-thumb-icon '.$zozo_menu_font_icon.'"></em>';
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $zozo_menu_cat_columns);
