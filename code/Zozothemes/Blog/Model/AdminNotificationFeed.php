...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = array();
        foreach($this->getZozothemesModules() as $key => $module) {
            $key = str_replace('Zozothemes_', '', $key);
    ...
    /**
     * Get Zozothemes Modules Info
     *
     * @return $this
     */
    protected function getZozothemesModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Zozothemes_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('zozothemes_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'zozothemes_admin_notifications_lastcheck');
        return $this;
    }
