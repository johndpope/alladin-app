...
class Magestore_RewardPoints_Model_Transaction extends Mage_Core_Model_Abstract
{
    ...
    public function getRewardAccount()
    {
        if (!$this->hasData('reward_account')) {
            $this->setData('reward_account',
                Mage::getModel('rewardpoints/customer')->load($this->getRewardId())
            );
        }
    ...
    public function createTransaction($data = array())
    {
        ...
        if ($rewardAccount->getPointBalance() + $this->getPointAmount() < 0) {
            ...
            if($this->getData('creditmemo_holding') && $rewardAccount->getHoldingBalance() + $this->getPointAmount() >= 0){}
                ...
                if($this->getData('creditmemo_transaction')) Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Account balance of Customer is not enough to take points back.'));
                throw new Exception(
    ...
    public function completeTransaction()
    {
        if (!$this->getId() || !$this->getCustomerId()
            ...
            || !in_array($this->getStatus(), array(self::STATUS_PENDING, self::STATUS_ON_HOLD))
        ) {
        ...
        $maxBalance = (int)Mage::getStoreConfig(self::XML_PATH_MAX_BALANCE, $this->getStoreId());
        if ($maxBalance > 0 && $this->getRealPoint() > 0
    ...
    {
        if (!$this->getId() || !$this->getCustomerId()
            || !$this->getRewardId() || $this->getPointAmount() <= $this->getPointUsed()
            || $this->getStatus() > self::STATUS_COMPLETED || !$this->getStatus()
    ...
    public function sendBeforeExpireEmail()
    {
        ...
        Mage::getModel('core/email_template')
            ->setDesignConfig(array(
                ...
                Mage::getStoreConfig(self::XML_PATH_EMAIL_BEFORE_EXPIRE_TPL, $store),
                Mage::getStoreConfig(self::XML_PATH_EMAIL_SENDER, $store),
                $customer->getEmail(),
                $customer->getName(),
                array(
