...
/**
 * Rewardpoints Earning Adminhtml Controller
 * 
 * @category    Magestore
 * @package     Magestore_RewardPoints
 * @author      Magestore Developer
 */
class Magestore_RewardPoints_Adminhtml_EarningController extends Mage_Adminhtml_Controller_Action
{
    /**
     * init layout and set active for current menu
     *
     * @return Magestore_RewardPoints_Adminhtml_EarningController
     */
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $rateId == 0) {
            ...
            $this->_setActiveMenu('rewardpoints/earning');
            ...
            $this->_addContent($this->getLayout()->createBlock('rewardpoints/adminhtml_earning_edit'))
                ->_addLeft($this->getLayout()->createBlock('rewardpoints/adminhtml_earning_edit_tabs'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                if (!$model->getDirection()) {
                    $model->setDirection(Magestore_RewardPoints_Model_Rate::MONEY_TO_POINT);
                }
                $model->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('rewardpoints')->__('Earning rate was successfully saved')
                );
