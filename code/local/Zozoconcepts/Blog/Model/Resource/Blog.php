 ...
 */
class Zozoconcepts_Blog_Model_Resource_Blog
    ...
    public function _construct(){
        $this->_init('zozoconcepts_blog/blog', 'entity_id');
    }
    /**
    ...
    public function lookupStoreIds($blogId){
        ...
        $select  = $adapter->select()
            ->from($this->getTable('zozoconcepts_blog/blog_store'), 'store_id')
     ...
     * @return Zozoconcepts_Blog_Model_Resource_Blog
     * @author Zozoconcepts Hybrid
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Zozoconcepts_Blog_Model_Blog $object
    ...
    protected function _getLoadSelect($field, $value, $object){
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                array('blog_blog_store' => $this->getTable('zozoconcepts_blog/blog_store')),
                $this->getMainTable() . '.entity_id = blog_blog_store.blog_id',
                array()
            )
            ->where('blog_blog_store.store_id IN (?)', $storeIds)
            ->order('blog_blog_store.store_id DESC')
            ->limit(1);
        }
        return $select;
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object){
        ...
        $table  = $this->getTable('zozoconcepts_blog/blog_store');
        $insert = array_diff($newStores, $oldStores);
        $delete = array_diff($oldStores, $newStores);
        if ($delete) {
            $where = array(
                'blog_id = ?' => (int) $object->getId(),
                'store_id IN (?)' => $delete
            );
    ...
    protected function _initCheckUrlKeySelect($urlKey, $store){
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('es' => $this->getTable('zozoconcepts_blog/blog_store')),
                'e.entity_id = es.blog_id',
                array())
