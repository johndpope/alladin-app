...
class Zozoconcepts_Hybrid_Model_Compiler
{    
    /**
    ...
    public function extractor($node)
    {
        ...
        if (count($options) < count($modules)) {
            Mage::throwException(Mage::helper('hybrid')->__('There is not enough option(s) for specified module(s).'));
        }
        if (count($options) > count($modules)) {
            Mage::throwException(Mage::helper('hybrid')->__('There is not enough module(s) for specified option(s).'));
        }
        ...
        for ($i = 0, $len = count($tokens); $i < $len; $i++) {
            if (is_numeric($tokens[$i])) {
                ...
                $outputStack[] = $condition !== null 
                    ? $condition 
                    : Mage::throwException(Mage::helper('hybrid')->__('Specified operands are more than condition(s).'));
            }
            else {
