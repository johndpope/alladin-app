 ...
 *
 */
class Zozoconcepts_Hybrid_Lib_Data_Form_Element_Grid_Columns
	extends Varien_Data_Form_Element_Text
{
	...
	public function getElementHtml()
	{
		...
		$html .= '</div>';
		$html .= '<div class="validation-advice" style="display:none;" id="' . $mainValidationId . '">' . Mage::helper('megamenu')->__('Sum of the grid units should be equal %s', $this->_gridUnitMax) . '</div>';
		...
		//Scripts
		if (!Mage::registry('hybrid_admin_jquery'))
		{
			$jqueryUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_JS) . 'hybrid/jquery/jquery-admin.min.js';
			$html .= '<script type="text/javascript" src="' . $jqueryUrl . '"></script>';
			$html .= '<script type="text/javascript">jQuery.noConflict();</script>';
			Mage::register('hybrid_admin_jquery', 1);
		}
		$html .= '
