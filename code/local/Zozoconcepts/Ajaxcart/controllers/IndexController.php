...
class Zozoconcepts_Ajaxcart_IndexController extends Mage_Checkout_CartController
{
		...
		if($params['isAjax'] == 1){
			...
			try {
				...
				if (!$cart->getQuote()->getHasError()){
                    ...
                    $code  = $store->getCode();
                    $aspect_ratio = Mage::getStoreConfig("hybrid/category_page/aspect_ratio",$code);
                    $ratio_width = Mage::getStoreConfig("hybrid/category_page/ratio_width",$code);
                    $ratio_height = Mage::getStoreConfig("hybrid/category_page/ratio_height",$code);
                    ...
                    $product_image = '<div class="success-img-container">
					<div class="timer-wrap"><i class="icon icon-Timer"></i>
					<div class="timer">'.$autoclose.'</div>
					</div>
					<img src="'.$product_image_src.'" class="product-image img-responsive" alt=""/><p class="product-name">'.Mage::helper('core')->htmlEscape($product->getName()).'</p></div>';
					$message = '<div class="msg">'.$this->__("<h5 class='alert-msg'>Product added to cart</h5>").'</div>'.$product_image;
					...
					$minicartdrop = "";
                    if($this->getLayout()->getBlock('minicartdrop'))
                        $minicartdrop = $this->getLayout()->getBlock('minicartdrop')->toHtml();	
                    $cart_sidebar = "";
					...
					$response['minicartdrop'] = $minicartdrop;
