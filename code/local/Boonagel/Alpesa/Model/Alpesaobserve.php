...
class Boonagel_Alpesa_Model_Alpesaobserve {
    ...
    public function updateTarget($observer) {
        ...
        if ($allConditions->count() > 0) {
            ...
            $alpesaTotalActualPoints = Mage::helper('Boonagel_Alpesa')->sumColVals('alpesa/alpesapoints', array('status' => 'eq,1'), 'points');
        ...
        if ($logCount == 0) {
            ...
            if ($canAward == true) {
                ...
                $configData = Mage::helper('Boonagel_Alpesa')->getConfigData();
                 $amount = Mage::helper('Boonagel_Alpesa')->pointPriceConversion($configData,$condition->points_reward,'','pnt-curr'); 
