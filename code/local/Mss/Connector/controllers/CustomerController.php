...
class Mss_Connector_CustomerController extends Mage_Core_Controller_Front_Action {
	...
	*/
	...
	public function loginAction() {
		...
		if (Mage::getSingleton ( 'customer/session' )->isLoggedIn ()) {
			$session->logout ();
			$adminSession = Mage::getSingleton('core/session', array('name' => "frontend"));
	...
	public function registerAction() {
		...
		try {
			...
			if ($e->getCode () === Mage_Customer_Model_Customer::EXCEPTION_EMAIL_EXISTS) {
				...
				$message = $this->__( 'There is already an account with this email address.', $url );
 	...
 	public function getAddressAction()
 	{
 		try {
 			...
 			if($userid){
				...
				$currentADDDDD =  array();
				foreach ($currentAddress as $key => $value) {

					if($value['is_default_billing'] == 1)
					{
						array_push($currentADDDDD, $value);
					}
				...
				foreach ($currentAddress as $key => $value) {
					...
					$customerAddress = array('alladdress'=>$alladdress,'currentAddress'=>$currentADDDDD);
