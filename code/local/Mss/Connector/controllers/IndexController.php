	...
	public function getProductlist($products, $mod = 'product',$price_filter = array()) {
		...
		foreach($products as $product):
			...
			$productlist [] = array (
					...
					'regular_price_with_tax' => Mage::helper('tax')
        ->getPrice($product, $product->getPrice()),
					'final_price_with_tax' => Mage::helper('tax')
		...
		switch ($cmd) {
			...
			case 'catalog' : // OK
				...
				if(sizeof($productlist)){
					...
					echo json_encode ( array('id'=> $category->getId(),'category_image'=> $category->getImageUrl(),'products'=>$productlist ));
					exit;
				}
				else
					echo json_encode ( array('id'=> $category->getId(),'category_image'=> $category->getImageUrl(),'products'=> array() ));
