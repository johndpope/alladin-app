...
class Amasty_Acart_Model_Rule extends Mage_Rule_Model_Rule
{
        ...
        if (is_array($this->getSchedule())) {
            ...
            foreach ($schedule['row_index'] as $order) {
                ...
                if ($order != '_rowIdx_') { //skip first template row
                    ...
                    if (!array_key_exists('email_templates', $schedule)
                        || !array_key_exists($order, $schedule['email_templates'])
                        || !$this->isEmailTemplateExist($schedule['email_templates'][$order])
                    ) {
                        throw new Exception(
                            Mage::helper('amacart')->__('Email template is not exist')
                        );
                    }
                    ...
                    if (intval($schedule['use_rule'][$order]) == 1) {
                        ...
                        $saveItem = array(
                            ...
                            'schedule_id' => array_key_exists($order, $schedule['schedule_id'])
                                ? $schedule['schedule_id'][$order] : null
                        );
                    }
            ...
            foreach ($scheduleDbData as $scheduleDbItem) {
                $saveDataKey = $this->recursiveFindKeys(
                    $scheduleDbItem->getScheduleId(),
                    $saveData
                );

                if (($saveDataKey !== false) && array_key_exists($saveDataKey, $saveData)) {
                    $scheduleDbItem->addData($saveData[$saveDataKey]);

                    $scheduleDbItem->save();
                    unset($saveData[$saveDataKey]);
                } else {
             ...
             * @return mixed
             */
            $prepareForSave = function ($item) {
                if (is_array($item) && array_key_exists('schedule_id', $item)) {
            ...
            foreach (array_map($prepareForSave, $saveData) as $delayed_start => $config) {
     ...
     * @return bool
     */
    protected function isEmailTemplateExist($templateId)
    {
        if (!$templateId) {
            return false;
        }
        ...
        return $emailTemplate->getId() ? true : false;
    }
     ...
     */
    protected function recursiveFindKeys($needle, $haystack)
    {
        foreach ($haystack as $key => $value) {
            $current_key = $key;
            if ($needle === $value || (is_array($value) && $this->recursiveFindKeys($needle, $value) !== false)) {
                return $current_key;
            }
        }
